...
<html>
  <head>
	...
	function add(){
		$.post("system/toadd.do",{},function(data){
			$("#add").show();
			$("#add").html(data).show();
		});
	}
	function update(v){
		$.post("system/toupdate.do",{
			"id":v
		},function(data){
			$("#add").show();
			$("#add").html(data).show();
	...
	function toPage(p){
		...
		if(p<=0||p>total){
  		...
  		document.frm.action="system/list.do";
  ...
  <body>
 	<form name="frm" action="<%=path %>/system/list.do" method="get">
 	...
 	<div class="tai_right" id="tai_right">
    	<table width="100%" class="table_da">
          ...
          <c:forEach items="${data.objs}" var="o" varStatus="status">
		    ...
		    <input type="button" class="gaic" value="删除" onclick="window.location.href='<%=path %>/system/delete.do?id=${o.systemid }'">
		    </td></tr>
		    </c:forEach>
        </table><br>
    ...
    <div id="add" style="position:absolute;top:150px;left: 200px;"></div>
  </body>
</html>